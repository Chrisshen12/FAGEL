...
from dataset import load_Roman, load_Amazon, load_Cora, load_Reddit, load_Ogb_a, load_Ogb_p, load_Ogb_m
from model import GCN,TwoStageEnsemble
...
def run_batch_val(args):
	...
	#train_loader = ImportanceNeighborLoader(dataset, num_neighbors=neighbors, batch_size=batch_size_train, input_nodes=train_mask,shuffle=False)
	...
	#inference_loader = NeighborLoader(dataset, num_neighbors=neighbors, batch_size=batch_size_in,input_nodes=None,shuffle=False)
	...
	if args.inference:
		for i, state in enumerate(best_states):
			...
			save_path = os.path.join(save_dir, f"{args.dataset}_{args.batch_size}_hop{args.hop}_neighbors{args.neighbors}_seed{args.seed}_GCN_model_topk_{i}.pth")
			#save_path = os.path.join(save_dir, f"{args.dataset}_{args.batch_size}_hop{args.hop}_neighbors{args.neighbors}_SAGE_model_topk_{i}.pth")
	#       #save_path = os.path.join(save_dir, f"{args.dataset}_{args.batch_size}_hop{args.hop}_neighbors{args.neighbors}_GAT_model_topk_{i}.pth")
	#       #save_path = os.path.join(save_dir, f"{args.dataset}_{args.batch_size}_diverse_GCN_model_topk_{i}.pth")
	#       #save_path = os.path.join(save_dir, f"{args.dataset}_{args.batch_size}_gradient_GCN_model_topk_{i}.pth")
	#       #save_path = os.path.join(save_dir, f"{args.dataset}_{args.batch_size}_SAGE_model_topk_{i}.pth")
	...
	best_states = [torch.load(os.path.join(save_dir,f"{args.dataset}_{args.batch_size}_hop{args.hop}_neighbors{args.neighbors}_seed{args.seed}_GCN_model_topk_{i}.pth"))for i in range(selected_models)]
	#best_states = [torch.load(os.path.join(save_dir,f"{args.dataset}_{args.batch_size}_hop{args.hop}_neighbors{args.neighbors}_SAGE_model_topk_{i}.pth"))for i in range(selected_models)]
	#best_states = [torch.load(os.path.join(save_dir,f"{args.dataset}_{args.batch_size}_hop{args.hop}_neighbors{args.neighbors}_GAT_model_topk_{i}.pth"))for i in range(selected_models)]
	#best_states = [torch.load(os.path.join(save_dir,f"{args.dataset}_{args.batch_size}_diverse_GCN_model_topk_{i}.pth"))for i in range(args.topk)]
	#best_states = [torch.load(os.path.join(save_dir,f"{args.dataset}_{args.batch_size}_gradient_GCN_model_topk_{i}.pth"))for i in range(args.topk)]
	single_model_accs = []
...
def run_batch_val_efficient(args):
	...
	if args.inference:
		for epoch in range(args.epochs):
			...
			#print(f"Val Acc: {val:.4f}, patience: {patience}, Time: {elapsed_time:.4f}s, Peak GPU Mem: {torch.cuda.max_memory_allocated(device) / 1024**2:.2f} MB")
			#print(f"Val Acc: {val:.4f}, patience: {patience}, Time: {end_time - start_time:.4f}s, Peak GPU Mem: {torch.cuda.max_memory_allocated(device) / 1024**2:.2f} MB")
			#print(f"Val Acc: {val:.4f}, patience: {patience_counter}")
	print(f"Total epochs run: {total_epochs_run}")
	print(f"Total training time: {total_train_time:.2f} seconds")
	# print(f"Average time per epoch: {total_train_time / total_epochs_run:.2f} seconds")
...
def weighted_average_models(args):
	if args.dataset == 'Roman':
		dataset= load_Roman(seed=args.seed)
	elif args.dataset == 'Amazon':
		dataset= load_Amazon(seed=args.seed)
	...
	model = TwoStageEnsemble(num_models=selected_models,out_dim=num_classes,hidden_dim=args.hidden2).to(device)
...
def main(args):
	...
	#configs_batch = [(5,2),(5,3),(4,4),(5,5)]
	#configs_batch = [(4,2),(4,3),(3,3),(3,5)]
	# for d in dataset:
	# 	for hop, neighbors in configs_batch:
			
	# 			args.dataset = d
	# 			#args.batch_size = 0.5

	# 			print("=" * 60)
	# 			print(f"Testing {d}: init({hop},{neighbors})")
	# 			#args.dataset = d
	# 			#args.batch_size = b
	# 			args.hop2 = hop
	# 			args.neighbors2 = neighbors
	...
	# 		args.patience = 100
	# 		args.dropout = 0.5
	# 		args.batch_size = 0.001
	# 	elif d =='Amazon':
	# 		args.patience = 100
	# 		args.batch_size = 0.001
	# 	else:
	# 		args.batch_size = 0.01
	# 	print("=" * 60)
	# 	print(f'{d} SAGE')
	# 	run_batch_val(args)
